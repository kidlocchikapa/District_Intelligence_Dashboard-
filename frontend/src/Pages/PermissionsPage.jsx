        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm h-[600px] flex flex-col">
            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              ...
              {isLoadingUsers && <RotateCw size={14} className="animate-spin text-slate-400" />}
            </div>
              ...
              {filteredUsers.map(user => (
                <button
                  ...
                  className={`w-full text-left p-4 flex items-center justify-between group transition-all border-b border-slate-50 last:border-0 ${
                    selectedUserId === user.id ? 'bg-slate-900 text-white' : 'hover:bg-slate-50'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${
                      selectedUserId === user.id ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-600'
                    }`}>
                      {user.fullName.charAt(0)}
                    </div>
                    <div>
                      <div className={`text-sm font-bold ${selectedUserId === user.id ? 'text-white' : 'text-slate-900'}`}>{user.fullName}</div>
                      <div className={`text-[10px] font-medium ${selectedUserId === user.id ? 'text-white/60' : 'text-slate-500'}`}>{user.email}</div>
                    </div>
                  </div>
                  <ChevronRight size={14} className={selectedUserId === user.id ? 'text-white/40' : 'text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity'} />
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-8">
          {selectedUser ? (
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">{selectedUser.role.replace('_', ' ')}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 p-6 relative">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-2 text-slate-400">
                    <ShieldAlert size={16} />
                    <span className="text-[11px] font-bold uppercase tracking-widest">Departmental Data Access Control</span>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50/50">
                          <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Department</th>
                          <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-center">Read Access</th>
                          <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-center">Write Access</th>
                          <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] text-center">Recompute</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {permissions.map(perm => (
                          <tr key={perm.department} className="hover:bg-slate-50/30 transition-colors">
                            <td className="px-6 py-5">
                              <div className="flex items-center gap-3">
                                <div className="h-2 w-2 rounded-full bg-slate-300" />
                                <span className="text-sm font-bold text-slate-700 capitalize">{perm.department.replace('_', ' ')}</span>
                              </div>
                            </td>
                            <td className="px-6 py-5 text-center">
                              <PermissionToggle 
                                checked={perm.canRead} 
                                onChange={() => handleTogglePermission(perm.department, 'canRead')} 
                              />
                            </td>
                            <td className="px-6 py-5 text-center">
                              <PermissionToggle 
                                checked={perm.canWrite} 
                                onChange={() => handleTogglePermission(perm.department, 'canWrite')} 
                              />
                            </td>
                            <td className="px-6 py-5 text-center">
                              <PermissionToggle 
                                checked={perm.canRecompute} 
                                onChange={() => handleTogglePermission(perm.department, 'canRecompute')} 
                              />
                            </td>
                          </tr>
                        ))}
                  ...
                  {selectedUser.role === 'super_admin' && (
                    <div className="p-4 rounded-xl bg-amber-50 border border-amber-100 flex items-start gap-3">
                      ...
                      <p className="text-xs text-amber-800 font-medium leading-relaxed">
                        <strong>Super Admin Bypass:</strong> This user has global access permissions. Any restrictions defined here will be bypassed as long as their role is set to Super Admin.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl h-full flex flex-col items-center justify-center p-12 text-center text-slate-400">
              ...
              <p className="font-bold text-sm">Select a user to manage their system permissions</p>
            </div>
          )}
        </div>
      </div>
