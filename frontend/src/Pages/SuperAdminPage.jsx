...
export default function SuperAdminPage() {
  ...
  const userStats = useMemo(() => {
    ...
    const adminUsers = users.filter((user) => {
      ...
      return role === "super_admin" || role.includes("admin");
    }).length;
    const roleCount = new Set(
      users.map((user) => String(user.role || "").trim()).filter(Boolean),
    ).size;

    return {
      totalUsers: users.length,
      activeUsers,
      inactiveUsers: Math.max(users.length - activeUsers, 0),
      adminUsers,
      roleCount,
    };
  }, [users]);
  ...
  return (
    <div className="mx-auto flex h-full min-h-0 w-full max-w-[1600px] flex-col gap-5 overflow-y-auto px-3 py-4 sm:px-4 sm:py-5 md:px-6 md:py-6 lg:px-8 lg:py-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        {status && (
          <div className="animate-in fade-in slide-in-from-top-2 rounded-lg border border-emerald-100 bg-emerald-50 px-4 py-2 text-xs font-medium text-emerald-700 duration-300 lg:max-w-md">
            {status}
          </div>
        )}
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          },
          {
            ...
            note: `${userStats.inactiveUsers} inactive`,
            icon: CheckCircle2,
          },
          {
          ...
          },
          {
            ...
            value: userStats.roleCount,
            note: "Unique role labels",
            icon: Columns,
          },
        ].map((card) => {
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/50 p-3 px-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 md:justify-end">
            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">
              {users.length} users • {isLoading ? "loading..." : "up to date"}
            </div>
        ...
        <div className="min-h-0 flex-1 overflow-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-200">
