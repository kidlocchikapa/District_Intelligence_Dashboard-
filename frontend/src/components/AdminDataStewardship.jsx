  ...
  return (
    <div className="flex h-full min-h-[520px] flex-col overflow-hidden rounded border border-slate-200 !bg-white shadow-none md:min-h-[640px]">
      <div className="!bg-white">
        ...
        <div className="flex gap-2 overflow-x-auto !bg-white px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
          {tables.map((table) => {
      ...
      <main className="relative flex-1 !bg-white">
        ...
        <div className="space-y-3 overflow-auto p-3 sm:p-4 md:hidden">
          {!loading && records.length === 0 ? (
            ...
            records.map((record, idx) => (
              <article
                key={record[deptConfig.idKey] || record.school_id || idx}
              ...
              >
                <div className="space-y-2">
                  {mobileColumns.map((col) => (
                    <div key={col}>
                      ...
                      <p
                        className="mt-0.5 break-words text-sm font-semibold text-slate-700"
                ...
                {columns.length > mobileColumns.length ? (
                  <p className="mt-3 text-[11px] font-semibold text-slate-400">
                    +{columns.length - mobileColumns.length} more fields in desktop table view
                  </p>
                ) : null}
                ...
                <div className="mt-3 border-t border-slate-100 pt-3">
                  {selectedTable?.editable ? (
                    <button
                      ...
                      className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-800 transition-all hover:border-slate-900 hover:bg-slate-900 hover:text-white"
                    >
                      Edit record
                    </button>
                  ) : (
        ...
        <div className="hidden h-full overflow-auto md:block">
          <table className="w-full min-w-[1200px] border-collapse !bg-white text-left lg:min-w-[1500px]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-3 py-4 sm:px-4 sm:py-6">
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-4 sm:p-5">
              <div>
                <h3 className="text-lg font-bold text-slate-950 sm:text-xl">{displayValue(editingRecord.name)}</h3>
            ...
            <div className="grid gap-4 overflow-y-auto p-4 sm:gap-6 sm:p-5 lg:grid-cols-[1fr_280px]">
            ...
            <div className="flex flex-col-reverse justify-end gap-3 border-t border-slate-200 bg-slate-50 p-4 sm:flex-row">
              <button
                type="button"
                onClick={() => setEditingRecord(null)}
