...
// Upload Endpoint
router.post('/upload', authenticateToken, upload.single('dataset'), (req, res) => {
    if (!req.file) {
    ...
    console.log(`[ETL Trigger] Starting pipeline for uploaded file: ${uploadedFilename}`);

    // Spawn the Python ETL process
    // The python script lives in /app/etl/main.py
    const etlProcess = spawn(process.env.ETL_PYTHON_PATH || 'python3', [
        '/app/etl/main.py',
